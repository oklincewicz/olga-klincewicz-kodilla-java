...
public class TaskListDaoTestSuite {
    ...
    public void testFindByListName() {
        //Given
        TaskList taskList = new TaskList("JOHN LENNON", DESCRIPTION);
        taskListDao.save(taskList);
        String listName = taskList.getListName();

        //When
        List<TaskList> readTaskName = taskListDao.findByListName(listName);

        //Then
        Assert.assertEquals(1, readTaskName.size());

        //CleanUp
        int id = readTaskName.get(0).getId();
